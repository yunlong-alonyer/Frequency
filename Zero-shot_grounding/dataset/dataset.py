...
class RSNA2018_Dataset(Dataset):
    def __init__(self, csv_path, root='/home/supermicro/data/rsna/rsna'):
    ...
    def __getitem__(self, index):
        img_path = self.img_path_list[index] + '.dcm'
        ...
        if class_label == 1:
            ...
            for box in boxes:
                cc = box.split(';')
                seg_map[int(float(cc[1])):(int(float(cc[1])) + int(float(cc[3]))),
                int(float(cc[0])):(int(float(cc[0])) + int(float(cc[2])))] = 1
...
class SIIM_ACR_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seg_path = self.seg_root + self.img_path_list[
            index] + '.png'  # We have pre-processed the original SIIM_ACR data, you may change this to fix your data
