...
class MAVL(nn.Module):
    ...
    def __init__(self, config, ana_book, disease_book, concept_book):
        ...
        with torch.no_grad():
            ...
            self.concept_book = self.concept_book.view(self.n_disease, self.n_concepts,
    ...
    def forward(self, images, return_intermediate=True):
        ...
        if self.same_feature:
            ...
            logits = torch.matmul(F.normalize(out_features, dim=-1),
            ...
            logits_normal = torch.stack(
                [logits[:, k, k % self.n_concepts] for k in range(self.n_disease * self.n_concepts)], dim=-1)
