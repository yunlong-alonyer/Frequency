...
class Chestxray14_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(path_parts) >= 3:
            truncated_path_parts = path_parts[2:]
        else:
            truncated_path_parts = path_parts
        truncated_path_parts = '/'.join(truncated_path_parts)

        # img_path = f'{self.root}/{img_path}'
        img_path = f'{self.root}/{truncated_path_parts}'
...
class SIIM_ACR_Dataset(Dataset):
    def __init__(self, csv_path,root, is_train=True):
        ...
        self.img_root = f'{root}/dicom/'
        self.seg_root = f'{root}/mask/'
