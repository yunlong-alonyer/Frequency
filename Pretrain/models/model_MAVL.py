...
class MAVL(nn.Module):
    ...
    def __init__(self, config, ana_book, disease_book, concept_book, mode='train'):
        ...
        with torch.no_grad():
            ...
            self.ana_book = bert_model(input_ids=ana_book['input_ids'], attention_mask=ana_book['attention_mask'])
        ...
        self.use_freq_mask = config.get('use_freq_mask')
    ...
    # =======================================================
    # [核心修改 1]: 将 is_train 传入，并在 ResNet 内部插桩！
    # =======================================================
    def image_encoder(self, xis, is_train=False):
        batch_size, _, H, W = xis.shape

        if 'resnet' in self.model_name.lower():
            x = xis
            # 依次通过 ResNet 的各个模块: conv1->bn1->relu->maxpool->layer1->layer2->layer3
            for i, layer in enumerate(self.backbone):
                x = layer(x)
                # 索引 i==4 正好是执行完 `layer1` 后！
                # 此时特征图拥有了初级语义，且抛弃了像素级底噪。Shape: [B, 256, 56, 56]
                if i == 4 and is_train and getattr(self, 'use_freq_mask', False):
                    # 在隐空间施加幅值频域掩码！
                    x, _ = self.freq_masker(x)
            res_fea = x
        else:
    ...
    def forward(self, images, labels, smaple_index=None, is_train=True, no_cl=False, exclude_class=False):
        ...
        ''' Visual Backbone '''
        # 将 is_train 参数传递给 image_encoder
        x, x_global = self.image_encoder(images, is_train=is_train)
        ...
        logits = torch.matmul(F.normalize(x_global, dim=-1), F.normalize(query_embed_, dim=-1).T)

        contrast_labels = labels.clone()
        contrast_labels[(contrast_labels == -1) | (contrast_labels == 2)] = 0
        loss_global_i2t = torch.stack([
            self.sce(logits[:, k::self.n_concepts] * self.temp[i].exp(), contrast_labels) for i, k in
            enumerate(self.global_concept_idx)]).mean()
        loss_global_t2i = torch.stack([
            self.sce(logits[:, k::self.n_concepts].T * self.temp[i].exp(), contrast_labels.T) for i, k in
            enumerate(self.global_concept_idx)]).mean()
